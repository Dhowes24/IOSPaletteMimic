//
//  imageSelectSub-ViewModel.swift
//  PaletteMimic
//
...
extension imageSelectSubView {
    @MainActor class imageSelectSub_ViewModel: ObservableObject {
        
        var image: UIImage?

        var palette: [UInt32] = []
        @Published var colorifiedPalette: [Color] = Array(repeating: Color.white, count: 16)
        @Published var sortedArr: [UInt32] = []
        ...
        func UInt32_split_into_buckets(img: UIImage, img_arr: [UInt32], depth: Int, photoNum: Int) {
            ...
            if depth == 0 {
                self.sortedArr.append(contentsOf: img_arr)
            ...
            UInt32_split_into_buckets(img: img, img_arr: Array(sorted_arr[0...median_index]), depth: depth-1, photoNum: photoNum)
            UInt32_split_into_buckets(img: img, img_arr: Array(sorted_arr[median_index...]), depth: depth-1, photoNum: photoNum)
        ...
        func UInt32_median_cut_quantize(img_arr: [UInt32], photoNum: Int) {
            ...
            self.palette.append(full_color)
            self.colorifiedPalette.removeFirst()
            let color = convert_to_color_single(palette: full_color)
            colorifiedPalette.append(color)
        }
...
//                                self.firstColorifiedPalette.append(newColor)
            ...
            print("am I here")
            if let image = image {
                let imageData = get_pixel_data(img: image)
                print("beginning 1st Image")

                UInt32_split_into_buckets(img: image, img_arr: imageData!, depth: 4, photoNum: 1)
